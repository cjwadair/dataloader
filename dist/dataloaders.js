...
(function (global, factory) {
	if (typeof define === "function" && define.amd) {
		...
		factory(mod.exports, global.localforage);
		global.dataloaders = mod.exports;
	}
	...
	exports.loadViaServiceWorker = loadViaServiceWorker;
	exports.loadImage = loadImage;
	exports.cacheFile = cacheFile;
	exports.loadCssFile = loadCssFile;
	exports.loadScript = loadScript;
	exports.cachePage = cachePage;
	exports.requestFromNetwork = requestFromNetwork;
	exports.detectElementAdded = detectElementAdded;
  ...
  */
	function setStorageInstances(cacheParams) {
		for (var key in cacheParams) {
	...
	function networkFirst(url, cacheName, cacheKey) {
		...
		var cacheInstance = storageInstances[cacheName];
 ...
 */
	function loadViaServiceWorker(url) {
		return new Promise(function (resolve) {
	...
	function loadImage(element, cacheName) {
		var loadAsBgImg = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

		var source = storageInstances[cacheName];
		var requestHandler = requestHandlers[source._config.requestMethod];
		var requestKey = element.getAttribute('dataloader-src');
 ...
 */
	function loadCssFile(url) {
		var cacheName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'staticFileStore';


		console.log('load css target is: ', url);

		var source = storageInstances['staticFileStore'];
		var requestHandler = requestHandlers[source._config.requestMethod];
		var requestKey = url;
		...
		if (requestKey && source) {
			...
			requestHandler(cacheName, requestKey).then(function (response) {
				var fileBlob = window.URL.createObjectURL(response[0]);
				el.href = fileBlob;
 ...
 */
	...
	function cachePage(urlPath, pageContent) {
		return new Promise(function (resolve) {
			...
			var data = storageInstances['pageStore'].setItem(urlPath, pageContent);
			resolve(data);
		});
	}

	function requestFromNetwork(url, cacheName) {
	...
	function requestFromCache(cacheName, cacheKey) {
		...
		return new Promise(function (resolve, reject) {
			...
			if ((typeof cacheName === 'undefined' ? 'undefined' : _typeof(cacheName)) === undefined) {
			...
			function getAllItems(cacheInstance) {
				return cacheInstance.length(function (err, numberOfKeys) {
					if (numberOfKeys > 0) {
						cacheInstance.iterate(function (value, key, iterationNumber) {
 ...
 */
	...
	function updateCache(cacheName, data, keyFieldValue) {
		//eslint-disable-line
	...
	function manageCacheSize(cacheName) {
		...
		if (cacheInstance._config.maxEntries !== null) {
			cacheInstance.length().then(function (numberOfEntries) {
				var entriesOverLimit = numberOfEntries - cacheInstance._config.maxEntries;
				var itemArray = [];
 ...
 */
	...
	function detectElementAdded(selector, fn) {
		//eslint-disable-line
